...
object JavaFormat : CodeFormat() {
    ...
    override fun formatEvaluation(evaluation: Evaluation<*>, indent: String): String = indent + when (evaluation) {
        ...
        is Array2DAccess<*> -> "${formatEvaluation(evaluation.array)}[${formatEvaluation(evaluation.rowIndex)}][${formatEvaluation(evaluation.colIndex)}]"

        is Array2DSize -> "${formatEvaluation(evaluation.array)}.length"
    ...
    override fun formatStatement(statement: Statement, indent: String): String = indent + when (statement) {
        ...
        is Array2DAssign<*> -> "${formatEvaluation(statement.array)}[${formatEvaluation(statement.rowIndex)}][${formatEvaluation(statement.colIndex)}] = " +
                formatEvaluation(statement.value)
    ...
    override fun formatVar(v: JVar<*>?, indent: String): String = indent + when {
        ...
        v is Array2DVar<*> -> {
            var string = "{"
            val elements = v.value.iterator()
            var outer = false   // to correctly format the list of arrays
            elements.forEach { arr ->
                if (outer) string += ", {"
                else string += "{"
                var inner = false   // to correctly format the values inside each array
                arr.forEach { innerArrVar ->
                    if (inner) string += ", ${formatVar(innerArrVar)}"
                    else string += formatVar(innerArrVar)
                    inner = true
                }
                outer = true
                string += "}"
    ...
    override fun formatVarType(type: VarType<*>, indent: String): String = indent + when (type) {
        ...
        is Array2DType<*> -> formatVarType(type.elementType) + "[][]"
        VarType.UNIT -> "void"
